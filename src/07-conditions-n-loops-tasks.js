...
function getSumBetweenNumbers(n1, n2) {
  return [...Array((n2 + 1) - n1)].reduce((acc, el, i) => acc + n1 + i, 0);
}
...
function doRectanglesOverlap(rect1, rect2) {
  const areTheyOverlapping = (start, end, position) => position >= start && position <= end;
  return (areTheyOverlapping(rect1.top, rect1.height, rect2.top)
  && areTheyOverlapping(rect1.left, rect1.width, rect2.left))
  || (areTheyOverlapping(rect2.top, rect2.height, rect1.top)
  && areTheyOverlapping(rect2.left, rect2.width, rect1.left));
}
...
function getDigitalRoot(num) {
  const result = String(num).split('').reduce((acc, el) => acc + +el, 0);
